...
class text:
    ...
    def creat(self):
        # 创建一个输入组件
        # self.aw = Label
        self.face = Label(relief=RAISED, font=('宋体', 17),height = 4,width=21, bg='white', anchor=NW,wraplength=300)
        self.face.pack(side=TOP, pady=10)
        self.face1 = Label(relief=RAISED, font=('宋体', 17), height= 4, width=21, bg='white', anchor=NW,wraplength=300,
justify = 'left')
        ...
        # b = Button(self.ct, text='7', font=('微软雅黑', 20), height=1, width=1, bd=1).pack(pady = 101,padx = 290)
        ...
        b.bind('<Button-1>', self.dj)
        # b = tkinter.Button(self.ct, text='拓展', font=('微软雅黑', 20), fg=('#4F4F4F'), bd=0.5)
        # b.place(x=280, y=230, width=70, height=55)
        if b['text'] == 'AC':
            b.bind('<Button-1>', self.clean)
        if b['text'] == '←':
            b.bind('<Button-1>',self.tuige)
